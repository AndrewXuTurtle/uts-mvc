...
<div class="container-fluid">
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        ...
        <a href="{{ route('pkm.show', $pkm->id) }}" class="btn btn-sm btn-secondary">
        ...
        <div class="card-body">
            <form action="{{ route('pkm.update', $pkm->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="col-md-8">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control @error('judul_pkm') is-invalid @enderror" 
                                   id="judul_pkm" name="judul_pkm" value="{{ old('judul_pkm', $pkm->judul_pkm) }}" required>
                            @error('judul_pkm')
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control @error('deskripsi') is-invalid @enderror" 
                                      id="deskripsi" name="deskripsi" rows="4" required>{{ old('deskripsi', $pkm->deskripsi) }}</textarea>
                            @error('deskripsi')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-4">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control @error('jenis_pkm') is-invalid @enderror" 
                                ...
                                <option value="PKM-R" {{ old('jenis_pkm', $pkm->jenis_pkm) == 'PKM-R' ? 'selected' : '' }}>PKM-R (Riset)</option>
                                <option value="PKM-K" {{ old('jenis_pkm', $pkm->jenis_pkm) == 'PKM-K' ? 'selected' : '' }}>PKM-K (Kewirausahaan)</option>
                                <option value="PKM-M" {{ old('jenis_pkm', $pkm->jenis_pkm) == 'PKM-M' ? 'selected' : '' }}>PKM-M (Pengabdian Masyarakat)</option>
                                <option value="PKM-T" {{ old('jenis_pkm', $pkm->jenis_pkm) == 'PKM-T' ? 'selected' : '' }}>PKM-T (Karsa Cipta)</option>
                                <option value="PKM-KC" {{ old('jenis_pkm', $pkm->jenis_pkm) == 'PKM-KC' ? 'selected' : '' }}>PKM-KC (Karya Inovatif)</option>
                                <option value="PKM-AI" {{ old('jenis_pkm', $pkm->jenis_pkm) == 'PKM-AI' ? 'selected' : '' }}>PKM-AI (Artikel Ilmiah)</option>
                                <option value="PKM-GT" {{ old('jenis_pkm', $pkm->jenis_pkm) == 'PKM-GT' ? 'selected' : '' }}>PKM-GT (Gagasan Tertulis)</option>
                            </select>
                            @error('jenis_pkm')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control @error('status') is-invalid @enderror" 
                                ...
                                <option value="Proposal" {{ old('status', $pkm->status) == 'Proposal' ? 'selected' : '' }}>Proposal</option>
                                <option value="Didanai" {{ old('status', $pkm->status) == 'Didanai' ? 'selected' : '' }}>Didanai</option>
                                <option value="Selesai" {{ old('status', $pkm->status) == 'Selesai' ? 'selected' : '' }}>Selesai</option>
                                <option value="Ditolak" {{ old('status', $pkm->status) == 'Ditolak' ? 'selected' : '' }}>Ditolak</option>
                            </select>
                            @error('status')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select class="form-control select2 @error('dosen_pembimbing_id') is-invalid @enderror" 
                                ...
                                @foreach($dosen as $d)
                                    <option value="{{ $d->id }}" {{ old('dosen_pembimbing_id', $pkm->dosen_pembimbing_id) == $d->id ? 'selected' : '' }}>
                                        {{ $d->nama }} - {{ $d->nidn }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <input type="number" class="form-control @error('dana') is-invalid @enderror" 
                                   id="dana" name="dana" value="{{ old('dana', $pkm->dana) }}" min="0" step="0.01">
                            @error('dana')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control @error('pencapaian') is-invalid @enderror" 
                                   id="pencapaian" name="pencapaian" value="{{ old('pencapaian', $pkm->pencapaian) }}" 
                ...
                <div class="form-group">
                    ...
                    @if($pkm->file_dokumen)
                        <div class="mb-2">
                            <a href="{{ asset('storage/' . $pkm->file_dokumen) }}" target="_blank" class="btn btn-sm btn-info">
                ...
                <div id="mahasiswa-container">
                    @foreach($pkm->mahasiswas as $index => $mhs)
                        ...
                        <div class="row align-items-end">
                            <div class="col-md-8">
                                ...
                                <select class="form-control select2-mahasiswa" name="mahasiswa_nim[]" required>
                                    ...
                                    @foreach($mahasiswa as $m)
                                        <option value="{{ $m->nim }}" {{ $mhs->nim == $m->nim ? 'selected' : '' }}>
                                            {{ $m->nim }} - {{ $m->nama }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-3">
                                ...
                                <select class="form-control" name="mahasiswa_peran[]">
                                    <option value="Ketua" {{ $mhs->pivot->peran == 'Ketua' ? 'selected' : '' }}>Ketua</option>
                                    <option value="Anggota" {{ $mhs->pivot->peran == 'Anggota' ? 'selected' : '' }}>Anggota</option>
                                </select>
                            </div>
                            <div class="col-md-1">
                                <button type="button" class="btn btn-danger btn-sm remove-mahasiswa" {{ $index == 0 ? 'disabled' : '' }}>
                ...
                <div class="d-flex justify-content-end">
                    <a href="{{ route('pkm.show', $pkm->id) }}" class="btn btn-secondary mr-2">
